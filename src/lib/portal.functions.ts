...
export const adminAcceptQuotationFn = createServerFn({ method: "POST" })
  .inputValidator((data: { id: number; remark: string; channel?: string }) => data)
  .handler(async ({ data }) => {
    await (await admin()).assertAdmin();
    return (await db()).adminAcceptQuotation(data);
  });
